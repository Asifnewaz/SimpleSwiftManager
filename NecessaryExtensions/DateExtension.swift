...
//  CashBaba
//
//  Created by Mamun Ar Rashid on 8/10/17.
...
public extension Date {
    ...
    func toTokenExpirationFormat() -> String {
        let formater = DateFormatter()
        formater.dateFormat = AppGlobal.dateFormatForToken
        return formater.string(from:self)
    }
    
    func toRCISParameterString() -> String {
       let formater = DateFormatter()
        formater.dateFormat = AppGlobal.paraDateFormat
        return formater.string(from:self)
    }
    func toRCISBillPayParameterString() -> String {
        let formater = DateFormatter()
        formater.dateFormat = AppGlobal.billPayParaDateFormat
        return formater.string(from:self)
    }
    
    func toRCISTranParameterString() -> String {
        let formater = DateFormatter()
        formater.dateFormat = AppGlobal.tranParaDateFormat
        return formater.string(from:self.adjust(hour: 12, minute: 0, second: 0)).replacingOccurrences(of: "PM", with: "").replacingOccurrences(of: "AM", with: "").replacingOccurrences(of: "pm", with: "").replacingOccurrences(of: "am", with: "").trim()
    }
    func toRCISPaylaterParameterString() -> String {
        let formater = DateFormatter()
        formater.dateFormat = AppGlobal.payLaterDateFormat
        return formater.string(from:self).trim()
    }
    ...
    func endOfDay() -> Date {
        return Calendar.current.date(byAdding: DateComponents(month: 0, day: 0), to: self.startOfMonth())!
    }
...
extension Date {
    ...
    func diff(otherDate: Date) -> DateComponents {
        let components = Set<Calendar.Component>([.second, .minute, .hour, .day, .month, .year])
        let differenceOfDate = Calendar.current.dateComponents(components, from: otherDate, to: self)
